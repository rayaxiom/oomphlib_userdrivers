...
//        time_stepper_pt());
  ...
  oomph_info <<"Prob Number of equations: " << assign_eqn_numbers() << std::endl; 
...
template<class ELEMENT>
void CubeProblem<ELEMENT>::dump_it()
{
  oomph_info << "RRRP1: Getting Jacobian" << std::endl;
  CRDoubleMatrix* cr_matrix_pt = new CRDoubleMatrix;
  DoubleVector residual;

  double t_getjac_start = TimingHelpers::timer();
  this->get_jacobian(residual,*cr_matrix_pt);
  double t_getjac_end = TimingHelpers::timer();
