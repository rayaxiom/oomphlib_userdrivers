...
//  Vector<DoubleVector> out_vector(nvectors);
  ...
  bool distributed = true;
  Vector <DoubleVector*> out_vector_pt(nvectors);
  for (unsigned veci = 0; veci < nvectors; veci++) 
  {
    out_vector_pt[veci] = new DoubleVector;
    LinearAlgebraDistribution dist(comm_pt,nn,distributed);
    out_vector_pt[veci]->build(dist);
  }
...
//  create_vectors_to_split(nrowarray,comm_pt,distributed,out_vector);
  ...
  // from out vectors.
  LinearAlgebraDistribution in_distribution(comm_pt,
                                            calculatednnz,
                                            distributed);
...
//  }
  ...
  DoubleVectorHelpers::split(in_vector,out_vector_pt,true);
