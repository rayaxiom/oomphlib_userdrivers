...
echo "mpirun -np 1 taskset -c 0 ${CGINPUT} ./${PROGRAM} --nblock1d ${NBLOCK1D} --nn 712" >> ${TEST_LIST}
echo "mpirun -np 2 taskset -c 0,12 ${CGINPUT} ./${PROGRAM}  --nblock1d ${NBLOCK1D} --nn 1007" >> ${TEST_LIST}
echo "mpirun -np 4 taskset -c 0-1,12-13 ${CGINPUT} ./${PROGRAM} --nblock1d ${NBLOCK1D} --nn 1425" >> ${TEST_LIST}
echo "mpirun -np 8 taskset -c 0-3,12-15 ${CGINPUT} ./${PROGRAM} --nblock1d ${NBLOCK1D} --nn 2015" >> ${TEST_LIST}
echo "mpirun -np 16 taskset -c 0-7,12-19 ${CGINPUT} ./${PROGRAM} --nblock1d ${NBLOCK1D} --nn 2850" >> ${TEST_LIST}
echo "mpirun -np 24 taskset -c 0-23 ${CGINPUT} ./${PROGRAM} --nblock1d ${NBLOCK1D} --nn 3491" >> ${TEST_LIST}
}
...
then
  ...
  PROGRAM_DIR=${PWD##*/}
  cd $CURRENT_DIR
  OOMPH_PROGRAM_DIR="/mnt/iusers01/mh01/mbax5ml3/mpi_debug_paranoid/user_drivers/$PROGRAM_DIR"
  SCRATCH_PROGRAM_DIR="/mnt/iusers01/mh01/mbax5ml3/scratch/mpi_debug_paranoid/user_drivers/$PROGRAM_DIR"
