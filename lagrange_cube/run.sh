...
DT="--dt 0.1"
NSPP="$DIST_PROB $PROB_ID $DOC_SOLN $VISC $REY $MAX_ITER $ITSTIMEDIR $SOLVER_TYPE $DT"
...
FAMG_COARSE="--f_amg_coarse 0" #RS - 1, Falgout - 6
...
echo $PARAM
make cube && rm -rf ./RESLT/* && \
mpirun -np 2 ./cube $PARAM #&& \
