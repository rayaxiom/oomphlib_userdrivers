...
rawmat=load(fname);
rawmat(:,1)=rawmat(:,1)+1; rawmat(:,2)=rawmat(:,2)+1;
Amat2=spconvert(rawmat); % put it in Amatr1
clear rawmat;
...
rawmat=load(fname);
rawmat(:,1)=rawmat(:,1)+1; rawmat(:,2)=rawmat(:,2)+1;
Amat3=spconvert(rawmat); % put it in Amatr1
clear rawmat;
...
nrow_local2=size(Amat2,1);
nrow_local3=size(Amat3,1);
...
fname=('residual14_np4r0'); % Change this!
rawres=load(fname);
res0=rawres(:,2); % Change this!
clear rawres;

fname=('residual14_np4r1'); % Change this!
rawres=load(fname);
res1=rawres(:,2); % Change this!
clear rawres;

fname=('residual14_np4r2'); % Change this!
rawres=load(fname);
res2=rawres(:,2); % Change this!
clear rawres;

fname=('residual14_np4r3'); % Change this!
rawres=load(fname);
res3=rawres(:,2); % Change this!
clear rawres;
...
cut_min = cut_max+1;
cut_max = cut_max + nrow_local2;
res2=res(cut_min:cut_max); % change this

cut_min = cut_max+1;
cut_max = cut_max + nrow_local3;
res3=res(cut_min:cut_max); % change this
...
fname='res14_np4r2'; % set this
nrow_local=nrow_local2; % set this
res=res2; % set this
fid=fopen(fname,'w+');
for i=1:nrow_local
   fprintf(fid,'%12.15f\n',res(i));
...
fname='res14_np4r3'; % set this
nrow_local=nrow_local3; % set this
res=res3; % set this
fid=fopen(fname,'w+');
for i=1:nrow_local
   fprintf(fid,'%12.15f\n',res(i));
...
cut_min = cut_max+1;
cut_max = cut_max + nrow_local2;
Arcm2 = Arcm(cut_min:cut_max,1:nrow);

cut_min = cut_max+1;
cut_max = cut_max + nrow_local3;
Arcm3 = Arcm(cut_min:cut_max,1:nrow);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Now output the val, col and row_pt for r2
Arcm = Arcm2; % Set this
nrow_local = nrow_local2; % Set this
fname='jac14_np4r2';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Now output the val, col and row_pt for r3
Arcm = Arcm3; % Set this
nrow_local = nrow_local3; % Set this
fname='jac14_np4r3';
